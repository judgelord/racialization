...
rules <- rules_racial_distinct_totals |>
   filter(documentType %in% c("Proposed Rule", "Rule")  ) |> 
  group_by(department_agency_acronym) |>
  summarise(n = sum(n, na.rm = T) ) |> 
  ungroup() |> 
  mutate(sub = str_remove(department_agency_acronym, ".*_"),
         main = str_remove(department_agency_acronym, "_.*")) 

n_rules_racial <- sum(rules$n)
...
rules %<>% 
  ...
  mutate(
    ...
    rules_n = n,
                n = sqrt(n),
                rules_norm = (n - mean(n))/sd(n) 
                  # percent = n/total,
                  # rules_percent_norm =  (percent - mean(percent))/sd(percent)
    ) %>%
  select(rules_n, rules_norm, 
         #rules_percent_norm, 
...
d <- full_join(nyt,
               p2025) |> distinct() |> 
  full_join(rules |> select(department_agency_acronym, rules_n, rules_norm)) |> 
  distinct()
...
max_x = max(d$MFL_norm, na.rm = T)
max_y = max(d$NYT_norm, na.rm = T)
min_x = min(d$MFL_norm, na.rm = T)
min_y = min(d$NYT_norm, na.rm = T)
...
d |> 
  ...
  annotate(x = min_x, y = max_y, geom = "text", 
           label = 'More Racialized in NYT', color = "red", hjust = 0) + 
  annotate(x = max_x, y = min_y, geom = "text", 
           label = 'More Racialized in MFL', color = "red", hjust = 1) + 
...
max_x = max(d$MFL_percent_norm, na.rm = T)
max_y = max(d$NYT_percent_norm, na.rm = T)
min_x = min(d$MFL_percent_norm, na.rm = T)
min_y = min(d$NYT_percent_norm, na.rm = T)
...
nyt_v_2025_cor <- cor.test(d$MFL_percent_norm, d$NYT_percent_norm)
...
max_x = max(d$rules_norm, na.rm = T)
max_y = max(d$NYT_norm, na.rm = T)
min_x = min(d$rules_norm, na.rm = T)
min_y = min(d$NYT_norm, na.rm = T)
...
rules_v_2025_cor <- cor.test(d$rules_norm, d$NYT_norm)
...
min_x = min(d$MFL_norm, na.rm = T)
min_y = min(d$rules_norm, na.rm = T)
...
d |> 
  ...
  aes(x = MFL_norm, 
    ...
    geom_line(aes(y = MFL_norm), color = "red")  + 
  annotate(x = min_x, y = max_y, geom = "text", 
           label = 'More Racialized in Rules', color = "red", hjust = 0) + 
  annotate(x = max_x, y = min_y, geom = "text", 
           label = 'More Racialized in MFL', color = "red", hjust = 1) + 
  #geom_smooth(method = "lm") + 
  geom_point() +    geom_label_repel(size =3,force_pull = 10, alpha = .5)  + 
...
rules_v_2025_cor <- cor.test(d$MFL_norm, d$rules_norm)
...
### Least racialized 
```{r}
d %>% distinct(department_agency_acronym, racialization) %>% arrange(racialization) %>% kablebox()
```
...
cor.test(d$rules_norm,  d$perceived_ideology_estimate)
cor.test(d$rules_norm, abs(d$perceived_ideology_estimate))
...
m_layoffs_ideo_mfl0 <-glm(layoffs ~ 
                          perceived_ideology_estimate*(MFL_norm>0) +
                  log(total_staff) + 
                  log(annual_budget_usd),
                  family=binomial(link="logit"),
                data=d )
