...
This file produces all of the numbers and figures used in the paper, except the trends over time figures, which are currently produced in `trends-nyt.qmd` and `trends-rules.qmd`.
...
The data required to run the code below and the code that produced it are available on [github.com/judgelord/racialization](https://github.com/judgelord/racialization/tree/master/data)
...
"mfl_data" in order of most racialized 
...
mfl |> arrange(-MFL_percent_norm) |> kablebox()
...
"nyt_data" in order of most racialized 
...
# from code/get_data_nyt.R  + docs/trends-nyt.qmd
load(here::here("data", "nyt.rda") ) 

nyt  |> arrange(-NYT_percent_norm) |> kablebox()
...
nyt |> 
  ...
  geom_col() + 
  labs(x = "Share with Racial Words", 
       y = "")
...
"rules_data" in order of most racialized 
...
rules  |> arrange(-rules_percent_norm) |>  kablebox()
...
d1 |> 
  ...
  labs(title = "Racialized Attention",
       x = "Scaled Mandate for Leadership Words",
...
d1 |> 
  ...
  geom_point() +    geom_label_repel(size =3,force_pull = 10, alpha = .5)  + 
  labs(title = "Racialized Attention as a Share of Total",
       x = "Scaled Share of Mandate for Leadership",
       y = "Scaled Share of Rules")
...
d <- d |>
  ...
  mutate(racialization = mean(c(MFL_percent_norm, 
                              ...
                              trim = 0,
                              na.rm = TRUE))
racialization_scores <- d
save(racialization_scores, file = here::here("data", "racialization_scores.rda"))
...
# split 
d |> 
  drop_na(perceived_ideology_estimate) |> 
  mutate(side = ifelse(perceived_ideology_estimate<0, "Liberal", "Conservative")) |> 
  ggplot() + 
    facet_wrap("side") + 
    theme_bw() +
  aes(x = abs(perceived_ideology_estimate), 
